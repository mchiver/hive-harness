...
- If you need to edit a file, read it first, plan the exact change, and make ONE edit only.
...
When editing files:
- **After successful edits** - Re-read the affected lines to confirm the change and update context
- **After "String not found" errors** - Immediately re-read the file to see current state before retrying
- **Before each edit** - If uncertain, re-read the specific lines to ensure current content matches expectations
- **Avoid duplicate edits** - If an edit appears to already be applied, don't retry it
...
The `.reference/definitions` folder contains concise definitions for concepts and terminology used in the project.
The `.reference/guides` folder contains concise guides for performing specific development tasks.
You are expected to have awareness of these files and be able to use them meaningfully.
